...
if(isset($_GET['experience_defense']))
{
    ...
    $stmt = $conn->prepare("INSERT INTO expsummary (id, experience_defense, experience_mtech, experience_phd, ongoing_project, completed_project, teaching_lab, outreach_activity) VALUES (?,?,?,?,?,?,?,?)");
    $stmt->bind_param("ssssssss", $_SESSION['id'], $experience_defense, $experience_mtech, $experience_phd, $ongoing_project, $completed_project, $teaching_lab, $outreach_activity);
    $stmt->execute();

}

$sql1 = "SELECT experience_defense,experience_mtech,experience_phd,ongoing_project,completed_project,teaching_lab,outreach_activity FROM `expsummary` WHERE id='".$_SESSION['id']."';";
